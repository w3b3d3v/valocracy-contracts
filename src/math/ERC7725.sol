...
import {IERC7725} from "./IERC7725.sol";
 ...
 */
abstract contract ERC7725 is IERC7725 {
    /**
    ...
    function expcurve(
        ...
        if (initialTimeframe > currentTimeframe) revert EXPCurveInvalidInitialTimeframe();
        if (initialTimeframe >= finalTimeframe) revert EXPCurveInvalidInitialTimeframe();
        if (curvature == 0 || curvature > 10_000 || curvature < -10_000) revert EXPCurveInvalidCurvature();
